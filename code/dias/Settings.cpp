...
int CSettings::GetSetting (UINT nID, SettingQuery nQueryType, bool bl, UINT nSubNo) {
	switch (nID) {
		case ID_PATCH_CONTRCOLOR:
			if (nQueryType == sqCurrent)	return (m_nPatchColor);
			break;
		case ID_PATCH_COLORMASK:
			if (nQueryType == sqCurrent)	return (m_nPatchColorMask);
			break;
	}
	return (-1);
}
