	...
	switch (m_nScale) {
		...
		case scaleLog:
			if (m_rCoef2 <= 0 || n <= 0)
				...
				r = m_rCoef1 * log (n) / log (m_rCoef2);
			break;
		case scaleExp:
			r = m_rCoef1 * pow (m_rCoef2, n);
			break;
	}
	return (r);
}
...
void CDensScale::Transform (ubyte *p$, ubyte *p, uvar32_64 len, ubyte bpp) {
	...
	switch (m_nScale) {
		...
		case scalePolynom:
			for (n = 0 ; n < len ; ++n) {
				if (bpp == 1)	p$[n] = ubyte(m_rCoef1 * pow (p[n], m_rCoef2));
				if (bpp == 2)	pw$[n] = uword(m_rCoef1 * pow (pw[n], m_rCoef2));
				if (bpp == 3)	pd$[n] = udword(m_rCoef1 * pow (pd[n], m_rCoef2));
			}
			break;
		case scaleLog:
			if (m_rCoef2 <= 0)
			...
			for (n = 0 ; n < len ; ++n) {
				if (bpp == 1)	p$[n] = (p[n] <= 0) ? 0 : ubyte(r * log (p[n]));
				if (bpp == 2)	pw$[n] = (pw[n] <= 0) ? 0 : uword(r * log (pw[n]));
				if (bpp == 3)	pd$[n] = (pd[n] <= 0) ? 0 : udword(r * log (pd[n]));
			}
			break;
		case scaleExp:
			for (n = 0 ; n < len ; ++n) {
				if (bpp == 1)	p$[n] = ubyte(m_rCoef1 * pow (m_rCoef2, p[n]));
				if (bpp == 2)	pw$[n] = uword(m_rCoef1 * pow (m_rCoef2, pw[n]));
				if (bpp == 3)	pd$[n] = udword(m_rCoef1 * pow (m_rCoef2, pd[n]));
			}
			break;
	}
}
