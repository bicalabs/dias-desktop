...
CPatch::CPatch (CImage* pImg, CString name, CString descr) {
	m_pImg = pImg;
...
CPatch& CPatch::operator= (const CPatch& obj) {
	...
	m_pbMask = NULL;
	m_pdwContent = m_pdwContour = NULL;
...
void CPatch::Serialize (CArchive& ar) {
	...
	if (ar.IsStoring ()) {
		...
		if (ct == 0) {		// No compression
			...
			if (m_pbMask)
				aimMemoryRelease (m_pbMask, "CPatch::Serialize", "m_pbMask");
			if (m_pdwContent)
				aimMemoryRelease (m_pdwContent, "CPatch::Serialize", "m_pdwContent");
			if (m_pdwContour)
				aimMemoryRelease (m_pdwContour, "CPatch::Serialize", "m_pdwContour");
...
void CPatch::Paint (CDC& dc) {
	...
	for (uvar32_64 dw = 0 ; dw < m_dwCntLen ; ++dw)
		dc.SetPixelV (SRCOFFSET_TO_COORDX(m_pdwContent[dw]),
		              SRCOFFSET_TO_COORDY(m_pdwContent[dw]),
		              m_pImg->GetPixel (m_pdwContent[dw]) & theSettings.GetSetting (ID_PATCH_COLORMASK, CSettings::sqCurrent, true, 0));
	for (dw = 0 ; dw < m_dwConLen ; ++dw)
		dc.SetPixelV (SRCOFFSET_TO_COORDX(m_pdwContour[dw]),
		              SRCOFFSET_TO_COORDY(m_pdwContour[dw]),
			      theSettings.GetSetting (ID_PATCH_CONTRCOLOR, CSettings::sqCurrent, true, 0));
}
