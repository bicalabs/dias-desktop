...
CMark::CMark (CImage* pImg, svar16_32 x, svar16_32 y, const CMark& deflt) {
	*this = CMark (pImg, CPoint (x, y), deflt);
}

CMark::CMark (CImage* pImg, CPoint& pos, const CMark& deflt) {
	m_pImg = pImg;
	m_pptPoints = NULL;
	*this = deflt;
	m_iPosX = pos.x;
...
CMark::CMark (CImage *pImg, CPoint& pos, MarkKernel krnl, uvar16_32 ksize, COLORREF kc, 
...
void CMark::Serialize (CArchive& ar) {
	...
	if (ar.IsStoring ()) {
		ar << m_iPosX << m_iPosY;
		ar << uchar(m_blSelected) << uchar(m_blVisible);
		ar << (uchar) (m_nKernel);
		ar << (uchar) (m_nShape);
		...
		m_blSelected = m_blVisible = false;
